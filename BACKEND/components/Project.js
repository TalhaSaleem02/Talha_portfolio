...
export default function Project({
    ...
    async function createProject(ev) {
        ...
        const data = { title, slug, images, videos, description, client, projectCategory, tags, livePreview, status };
        ...
        <>
            <form className='addWebsiteform' onSubmit={createProject}>
                ...
                <div className='w-100 flex flex-col flex-left mb-2'>
                    ...
                    <input type='text' placeholder=' Add Client Name' id="client" value={client} onChange={ev => setClient(ev.target.value)} />
                </div>
                ...
                <label htmlFor='projectCategory'>Select Category (for multiple select press ctr + mouse left key)</label>
                    <select onChange={(e) => setProjectCategory(Array.from(e.target.selectedOptions, (option) => option.value))} value={projectCategory} name='category' id='category' multiple>
